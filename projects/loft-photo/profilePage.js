...
export default {
  async setUser(user) {
    ...
    const photosTab = document.querySelector('.component-user-photo');
    const friendsTab = document.querySelector('.document-profile-mode-friends')
  ...
  async showPhotos() {
    ...
    for (const photo of photos.item) {
  ...
  async showFriends() {
    ...
    for (const friend of friends.item) {
      const element = document.createElement('div');
  ...
  handleEvents() {
    ...
    .addEventListener('click', async (e) => {
      ...
      if (friendId) {
        ...
        const photo = model.getRandomElement(friendsPhotos.item);
        const size = model.findSize(photo);
